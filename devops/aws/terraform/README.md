# AWS Deployment for Erlang with Terraform
...
Create an SSH key pair named, e. g. `erlgame-web-ec2` by visiting the [AWS Key Pair page](https://sa-east-1.console.aws.amazon.com/ec2/home?region=sa-east-1#KeyPairs:). Save the private key in your local SSH folder (`~/.ssh`). The name `erlgame-web-ec2` will be used by this file `devops/terraform/modules/standard-account/variables.tf` within terraform templates.
...
* *__deployex-erlgame-prod-secrets__*
 ...
 *  *__erlgame-stage-otp-tls-ca__*, *__erlgame-stage-otp-tls-key__*, *__erlgame-stage-otp-tls-crt__*:
...
*__PS__*: make sure you have the pair erlgame-web-ec2.pem saved in `~/.ssh/`

```bash
ssh -i "erlgame-web-ec2.pem" ubuntu@ec2-52-67-178-12.sa-east-1.compute.amazonaws.com
ubuntu@ip-10-0-1-56:~$
```
...
ca.crt  erlgame.crt erlgame.key deployex.crt  deployex.key
```
...
vi /etc/nginx/sites-available/default
 ...
           ssl_certificate /etc/letsencrypt/live/erlgame.com/fullchain.pem; # managed by Certbot
           ssl_certificate_key /etc/letsencrypt/live/erlgame.com/privkey.pem; # managed by Certbot
