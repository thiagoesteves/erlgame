...
resource "aws_security_group" "ec2_security" {
  name        = "erlgame-${var.account_name}-ec2-security-group"
...
resource "aws_iam_role" "ec2_iam_role" {
  name               = "erlgame-${var.account_name}-instance-role"
...
resource "aws_iam_role_policy_attachment" "attach-secrets" {
  role       = aws_iam_role.ec2_iam_role.name
  policy_arn = aws_iam_policy.erlgame_secrets_manager_policy.arn
}
...
resource "aws_iam_instance_profile" "erlgame_node" {
  name = "erlgame-${var.account_name}-ec2-profile"
  role = aws_iam_role.ec2_iam_role.name
}
...
resource "aws_instance" "ec2_erlgame_instance" {
  ami                         = data.aws_ami.ubuntu.id
  instance_type               = "${var.ec2_instance_type}"
  key_name                    = "${var.aws_key_name}"
  vpc_security_group_ids      = [aws_security_group.ec2_security.id]
  subnet_id                   = aws_subnet.public_subnet.id
  iam_instance_profile        = aws_iam_instance_profile.erlgame_node.name
  associate_public_ip_address = true
  user_data                   = data.cloudinit_config.server_config.rendered
  user_data_replace_on_change = true

  tags = {
    Name = "erlgame-${var.account_name}-instance"
  }
  lifecycle {
