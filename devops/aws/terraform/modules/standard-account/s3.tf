...
variable "s3_folders" {
  ...
  default     = ["dist/erlgame", "versions/erlgame"]
}

resource "aws_s3_bucket" "distribution" {
  bucket = "erlgame-${var.account_name}-distribution"
...
resource "aws_iam_policy" "s3_distribution_bucket_policy" {
  name = "erlgame-${var.account_name}-s3-distribution-bucket"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:aws:s3:::erlgame-${var.account_name}-distribution"
      },
      {
        Action = [
          "s3:GetObject",
