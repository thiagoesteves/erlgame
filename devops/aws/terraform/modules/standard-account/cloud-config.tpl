#cloud-config
#
#  Cloud init template for EC2 erlgame instances.
#
...
write_files:
  - path: /home/ubuntu/install-otp-certificates.sh
    ...
    content: |
      ...
      aws secretsmanager get-secret-value --secret-id erlgame-${account_name}-otp-tls-ca | jq -r .SecretString > /usr/local/share/ca-certificates/ca.crt
      aws secretsmanager get-secret-value --secret-id erlgame-${account_name}-otp-tls-key | jq -r .SecretString > /usr/local/share/ca-certificates/deployex.key
      aws secretsmanager get-secret-value --secret-id erlgame-${account_name}-otp-tls-key | jq -r .SecretString > /usr/local/share/ca-certificates/erlgame.key
      aws secretsmanager get-secret-value --secret-id erlgame-${account_name}-otp-tls-crt | jq -r .SecretString > /usr/local/share/ca-certificates/deployex.crt
      aws secretsmanager get-secret-value --secret-id erlgame-${account_name}-otp-tls-crt | jq -r .SecretString > /usr/local/share/ca-certificates/erlgame.crt
      echo "[OK]"
  - path: /home/ubuntu/deployex.yaml
    ...
    content: |
      ...
      port: 5001
      release_adapter: "s3"
      release_bucket: "erlgame-${account_name}-distribution"
      secrets_adapter: "aws"
      secrets_path: "deployex-erlgame-${account_name}-secrets"
          ...
          "logs_collected": {
            "files": {
              "collect_list": [
                ...
                {
                    "file_path": "/var/log/erlgame/erlgame-*-stdout.log",
                    "log_group_name": "${log_group_name}",
                    "log_stream_name": "{instance_id}-erlgame-stdout-log",
                    "timezone": "UTC",
                    "timestamp_format": "%H: %M: %S%Y%b%-d"
                },
                {
                    "file_path": "/var/log/erlgame/erlgame-*-stderr.log",
                    "log_group_name": "${log_group_name}",
                    "log_stream_name": "{instance_id}-erlgame-stderr-log",
                    "timezone": "UTC",
                    "timestamp_format": "%H: %M: %S%Y%b%-d"
                }
              ]
